...
def random_pair(x, constraint):
    """
    Generate a random instance within the given constraints.
    x: current instance (unused here, but kept for compatibility)
    constraint: np.array of shape (d,2), lower and upper bounds per feature
    """
    lower = constraint[:, 0]
    upper = constraint[:, 1]
