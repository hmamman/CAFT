...
def max_diff(x, similar_x, model):
    ...
    def distance(x_new):
        return np.sum(np.square(y_pred_proba - model.predict_proba([x_new])))
    max_dist = 0.0
...
def find_pair(x, similar_x, model):
    ...
    if(pairs.shape[0]==0):
        pairs = np.append(pairs, [similar_x[0]], axis=0)
        selected_p = random_pick([1.0 / (pairs.shape[0]+0.001)] * pairs.shape[0])
    else:
